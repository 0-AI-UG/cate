...
// HEADLESS sessions.
//
// Cate Agent sessions reuse the normal cate-agent machinery but are keyed with a
// `cate-agent-` panelId and never get an CateAgentPanel: agentStore routes their
...
// only one that reaches pi (CodingCreateOptions.systemPrompt is not plumbed to it).
...
export async function interruptCateAgent(panelId: string): Promise<void> {
  try {
    await codingClient.interrupt(panelId)
  } catch (err) {
...
export async function disposeCateAgent(panelId: string): Promise<void> {
  try {
    await codingClient.dispose(panelId)
  } catch (err) {
