// Coverage for CateAgentPanel's additive loop-chat branch. The panel hosts N coding
...
// (beginCodingCreate:false makes createCateAgent bail before any IPC). The loop branch
// under test doesn't touch the registry at all.
vi.mock('./codingSessionRegistry', () => ({
  getCateAgentPanelSession: () => undefined,
  saveCateAgentPanelSession: vi.fn(),
  disposeCodingChats: vi.fn(),
  disposeCodingChat: vi.fn(),
  resolvePanelChats: () => ({ refs: [], toResume: [] }),
  beginCodingCreate: () => false,
  endCodingCreate: vi.fn(),
...
describe('CateAgentPanel loop-chat branch', () => {
  ...
  it('flips the body to the loop view when "New loop chat" is chosen, and back', async () => {
    ...
    click(plus)
    // The chooser portals to <body> (shared NewChatChooser), so query the document.
    const loopItem = Array.from(document.body.querySelectorAll('button[role="menuitem"]')).find(
