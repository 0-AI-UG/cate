// Coverage for switchCateAgentWorktree — the confirm gate in front of the agent
...
import { switchCateAgentWorktree } from './CateAgentPanel'
...
describe('switchCateAgentWorktree', () => {
  it('asks before a switch that would drop a chat with messages, and does not write on cancel', async () => {
    const ok = await switchCateAgentWorktree({ ...base, chatCount: 1, hasMessages: true })
  ...
  it('asks when more than one chat is open even with no messages', async () => {
    await switchCateAgentWorktree({ ...base, chatCount: 2, hasMessages: false })
  ...
  it('switches silently with a single empty chat', async () => {
    const ok = await switchCateAgentWorktree({ ...base, chatCount: 1, hasMessages: false })
  ...
  it('never asks when the target resolves to the current checkout', async () => {
    const ok = await switchCateAgentWorktree({
      ...base,
