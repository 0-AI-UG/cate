...
// but targets the Cate Agent's OWN agent dir (cate-agent-loop) — normal agent
...
function sourceDir(): string | null {
  return findSourceDir([
    path.join(app.getAppPath(), 'src', 'cateAgent', 'extensions', 'cate-agent-tools'),
...
export async function installCateAgentToolsExtension(runtime: Runtime, cwd: string, variant: CodingDirVariant = 'cateAgent'): Promise<void> {
