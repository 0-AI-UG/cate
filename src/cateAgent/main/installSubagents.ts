...
// a workspace's cate-agent dir on first use. Pi auto-discovers extensions from
...
// src/cateAgent/extensions/subagent/ (copied from the pi-coding-agent npm package's
...
// <cwd>/.cate/cate-agent/ on the host that runs pi):
...
function subagentSourceDir(): string | null {
  return findSourceDir([
    path.join(app.getAppPath(), 'src', 'cateAgent', 'extensions', 'subagent'),
