...
// until app restart (nothing re-ran ensureLocalCompanion, and companion:ensure
// rejects local connections). retryLocal re-attempts the connect on demand.
describe('CompanionManager retryLocal', () => {
  afterEach(() => { vi.restoreAllMocks() })

  /** Prime localOpts the way ensureLocalCompanion would at startup. */
  function primeLocalOpts(mgr: CompanionManager): void {
    ;(mgr as unknown as { localOpts: unknown }).localOpts = { root: os.homedir() }
  }

  test('fails clearly when the local companion was never initialised', async () => {
    const mgr = new CompanionManager()
  ...
  test('is a no-op when LOCAL is already connected', async () => {
    ...
    primeLocalOpts(mgr)
    await mgr.connect(LOCAL_COMPANION_ID, new FakeTransport(), { install: true })
    const ensureSpy = vi.spyOn(mgr, 'ensureLocalCompanion')
  ...
  test('relaunches LOCAL after a failed startup connect and resolves once live', async () => {
    const mgr = new CompanionManager()
    primeLocalOpts(mgr)
    // Stand in for ensureLocalCompanion's transport construction (which needs a
    // real tarball): kick the same connect() it would, over a fake transport.
    vi.spyOn(mgr, 'ensureLocalCompanion').mockImplementation(() => {
      void mgr.connect(LOCAL_COMPANION_ID, new FakeTransport(), { install: true }).catch(() => {})
    })
    expect(mgr.has(LOCAL_COMPANION_ID)).toBe(false) // the startup connect failed
    const res = await mgr.retryLocal()
    expect(res).toEqual({ ok: true })
    expect(mgr.isConnected(LOCAL_COMPANION_ID)).toBe(true)
  })

  test('surfaces the unreachable reason when no connect can even start (no tarball)', async () => {
    const mgr = new CompanionManager()
    primeLocalOpts(mgr)
    // ensureLocalCompanion with no transport available: emits unreachable and
    // registers nothing.
    vi.spyOn(mgr, 'ensureLocalCompanion').mockImplementation(() => {
      mgr.report(LOCAL_COMPANION_ID, 'unreachable', 'No local companion tarball/target available')
    })
    const res = await mgr.retryLocal()
    expect(res.ok).toBe(false)
  ...
  test('a retry whose connect fails resolves ok:false with the connect error', async () => {
    ...
    vi.spyOn(mgr, 'ensureLocalCompanion').mockImplementation(() => {
      ...
      void mgr.connect(LOCAL_COMPANION_ID, transport).catch(() => {})
    })
    const res = await mgr.retryLocal()
    expect(res.ok).toBe(false)
