...
test('harnessRoot resolves the existing T3 state directory over the runtime wire', async () => {
  const { remote } = loopback(daemonApi())
  const os = await import('node:os')
  expect(await remote.file.harnessRoot()).toBe(path.join(os.homedir(), '.cate', 'extensions', '.cate-t3'))
})
