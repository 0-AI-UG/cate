...
import { useDurableCodingChat } from '../chat/useDurableCodingChat'
...
export const CateAgentSidebarView: React.FC<{ wsId: string; rootPath: string }> = ({ wsId, rootPath }) => {
  ...
  // A coding chat's transcript lives in useAgentStore (its `messages` array is []),
  ...
  const codingMsgCount = useAgentStore((s) => (codingAgentKey ? s.panels[codingAgentKey]?.messages.length ?? 0 : 0))
