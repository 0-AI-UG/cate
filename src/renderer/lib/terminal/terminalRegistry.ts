 ...
 *
 * This is the canonical buffer->string extraction; it is exported for reuse by
 * the (currently 6) external call sites that hand-roll the same baseY+cursorY /
 * translateToString(true) / trailing-trim loop. Those sites live in
 * other-track-owned files and are intentionally NOT updated here.
 */
function captureScrollback(entry: { terminal: Terminal; scrollback?: string }): string | undefined {
  if (entry.scrollback) return entry.scrollback
  try {
    const buffer = entry.terminal.buffer.active
    // Capture scrollback + the active viewport (baseY rows of history plus the
    // cursor row), so a follow-output terminal keeps its freshest lines.
