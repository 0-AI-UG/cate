...
export function filesFromPatch(cwd: string, patch: string): AgentChangedFile[] {
  ...
  return sections.flatMap((section) => {
    const unquote = (s: string) => { try { return s.startsWith('"') ? JSON.parse(s) as string : s } catch { return '' } }
    ...
    const filePath = relativeFile(cwd, unquote(rawPath ?? '').replace(/^[ab]\//, ''))
    if (!filePath) return []
    const hunks = parseReviewPatch(section)
    const file = fileFromHunks(filePath, hunks, hunks.length ? 'patch' : 'unavailable', section)
    const oldPath = relativeFile(cwd, unquote(source ?? header?.[1] ?? '').replace(/^a\//, ''))
...
export function filesFromTool(cwd: string, toolName: string, inputValue: unknown, outputValue?: unknown): AgentChangedFile[] {
  ...
  const acpFiles = content.flatMap((value) => {
    ...
    const filePath = relativeFile(cwd, diff.path)
    if (!filePath) return []
  ...
  if (patch?.includes('*** Begin Patch')) {
    return patch.split(/(?=^\*\*\* (?:Add|Update|Delete) File: )/m).flatMap((section) => {
      ...
      const filePath = relativeFile(cwd, name)
      if (!filePath) return []
      const lines = section.split('\n').filter((line) => /^[+ -]/.test(line) && !line.startsWith('***'))
      const hunks = parseReviewPatch(`@@ -1 +1 @@\n${lines.join('\n')}`)
      return [fileFromHunks(filePath, hunks, 'fragment')]
    })
  }
  ...
  if (Array.isArray(input.changes)) return input.changes.flatMap((value) => {
    ...
    const filePath = relativeFile(cwd, change.path)
    if (!filePath) return []
    const diff = string(change.diff)
    return [fileFromHunks(filePath, diff ? parseReviewPatch(diff) : [], diff ? 'patch' : 'unavailable', diff)]
  })
  const filePath = relativeFile(cwd, input.file_path ?? input.filePath ?? input.path ?? output.filePath)
