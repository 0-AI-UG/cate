 ...
 * response body is already encoded for the posting CLI, so forwarding it to
 * stdout uses the CLI's native submit-hook context channel. Always exits 0 —
 * a hook failure must never surface into the user's agent turn. */
...
process.stdin.on('end', () => {
  ...
  const req = http.request(endpoint + '/hook', {
    ...
    res.on('end', () => { if (res.statusCode === 200 && output) process.stdout.write(output); process.exit(0) })
  })
...
export function createAgentHooksCapability(deps: AgentHooksDeps = {}): AgentHooksCapability {
  ...
  const encodedPromptContext = (agentId: AgentId, terminalId: string): string => {
    ...
    if (!context) return ''
    const hook = AGENTS.find((agent) => agent.id === agentId)?.promptContextHook
    if (hook === 'additional-context') {
      ...
      void (async () => {
        try {
          ...
          if (req.url === '/prompt-context') {
            ...
            const context = promptContexts.get(body.terminalId) ?? ''
            res.statusCode = 200
  ...
  return {
    ...
    setPromptContext(terminalId, context) {
      ...
      else promptContexts.delete(terminalId)
    },
