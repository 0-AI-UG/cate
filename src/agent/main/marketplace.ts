...
function piBinaryPath(): string {
  ...
  // In packaged builds the .bin/ symlink doesn't survive asar unpacking, so
  // resolve to the actual CLI entry point in the package.
  const directPath = path.join(root, 'node_modules', '@earendil-works', 'pi-coding-agent', 'dist', 'cli.js')
  if (fs.existsSync(directPath)) return directPath
  // Fallback to .bin symlink for dev mode
...
function runPi(args: string[]): Promise<{ ok: true } | { ok: false; error: string }> {
  return new Promise((resolve) => {
    ...
    // When resolved to the .js entry point (packaged build), run it via Node
    // since the shebang symlink may not be executable inside the asar unpack.
