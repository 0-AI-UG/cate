...
`category` is the functional grouping Settings → Extensions filters the catalog by. It answers "what do I use this for?", never "how is it built" — the frontend/server split is invisible to users. The list is deliberately short (a 20-category catalog filters no better than none), and anything unrecognised or missing falls back to **Other**:
